  ...
  describe ServiceProvider do
    ...
    describe "valid metadata" do
      ...
      it "loads the key info" do
        expect(sp.keys.first.encryption_methods.first.algorithm)
          .to eq KeyDescriptor::EncryptionMethod::Algorithm::AES128_CBC
        expect(sp.keys.first.encryption_methods.first.key_size).to eq 128
      end

      it "loads service provider attributes" do
        expect(sp.authn_requests_signed?).to be true
        expect(sp.want_assertions_signed?).to be true
      end
    end
  end
end
